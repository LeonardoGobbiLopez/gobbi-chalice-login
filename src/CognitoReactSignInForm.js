...
class CognitoReactSignInForm extends Component {
  ...
  onClick() {
    ...
    xmlHttp.open("GET", 'https://nw2bom1pb8.execute-api.us-east-1.amazonaws.com/api/user-pools', true); // true for asynchronous
    xmlHttp.setRequestHeader("Authorization", this.state.tokenId)
    xmlHttp.send(null);
  }
  ...
  render() {
    if (this.state.signedIn) {
      return (<div>
        ...
        <button className="btn-toggle" onClick={this.handleLogout}>Sair</button>
        <button onClick={this.onClick}>TEST API</button>
      </div>)
    }
